#------------------------------------------------------------------------------------------------------------------------
#                                                chalbhai PHISHING KIT
#------------------------------------------------------------------------------------------------------------------------
# Files flagged by this tag file have been identified as using the chalbhai template, or clones of the chalbhai template.
# chalbhai is a code base that has been used in several phishing kits over the years.
# For example, phishing kits leveraging chalbhai have targeted: Apple, American Express, DHL, GoDaddy, Chase Bank,
# Bank of America, Daum, Costco, WeTransfer, Scotia Bank, Sharepoint, K&H Bank, INTERAC, One Drive, and more.
#------------------------------------------------------------------------------------------------------------------------
#                                                Associated Kit URLs
#------------------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------------
#                                                Associated Kit Functions
#------------------------------------------------------------------------------------------------------------------------
name=chalbhai
id=chalbhai
id=aramsejao
id=bhaagjoa
id=dafahoja
id=puranakhtab
name=bhoolgya
name=gulbahar
name=khannalao
name=khanalao
name=mundachok
name=alvisab
onsubmit="return validateANZ()"
#------------------------------------------------------------------------------------------------------------------------
#                                                Associated Kit Authors
#------------------------------------------------------------------------------------------------------------------------
Built By HustleLogic
Onye Mpa X
keith.luis
keithmoraga20
#------------------------------------------------------------------------------------------------------------------------
#                                                Associated Kit Files
#------------------------------------------------------------------------------------------------------------------------
surf.php
surf1.php
surf2.php
surf3.php
surf4.php
surf5.php
surf6.php
surf7.php
surf8.php
surf9.php
fast.php
fast1.php
fast2.php
fast3.php
fast4.php
fast5.php
fast6.php
fast7.php
fast8.php
fast9.php
need.php
need1.php
need2.php
need3.php
need4.php
need5.php
need6.php
need7.php
need8.php
need9.php
#------------------------------------------------------------------------------------------------------------------------
#                                                Misc. Items
#------------------------------------------------------------------------------------------------------------------------
chalbhai
galanchd